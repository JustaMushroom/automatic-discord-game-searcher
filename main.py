...
def search(queue_out, event_out, term_event_out, term_event_in):
	...
	for item in datajson:
		try:
			...
			if r.status_code == 404: # If SKU is reported as non-working
				pass
			elif r.status_code == 200: # If SKU is reported as working
				workingSKUS.append(item)
			elif r.status_code == 429: # If Rate-limited
				wait(10)
			else: # If unknown response code
				pass
			SKUCount += 1
